...
- **What it does:** Multi-provider fallback for the agentic tutor. `buildModelChain(tier)` returns the providers whose keys are set: the `'strong'` tier (real questions, routed by `api/_lib/taskRouter.js`) runs **Gemini 3.5 Flash → Gemini 3.5 Flash-Lite → Groq gpt-oss-120b → OpenRouter `openrouter/free`**; the `'light'` tier (bare greetings/acks) skips straight to Flash-Lite. Both model ids are env-overridable (`GEMINI_MODEL_STRONG` / `GEMINI_MODEL_LIGHT`), and the Gemini entries carry `thinkingConfig.thinkingBudget: 0` so no caller can forget it. `streamTextWithFallback({ chain, ...streamTextOpts }, onText)` streams text and, if a provider rejects a request *before the first byte*, transparently retries on the next one.
- **Why:** Groq's free tier caps at 8K tokens/request → `413` on big/agentic requests. Gemini's free tier (supports tools) has far more per-request headroom, so those big requests fit; Groq stays a fast fallback; OpenRouter free is a best-effort last resort (no SLA, models can vanish). NOTE: `gemini-3.5-flash` is a paid model ($1.50/$9.00 per 1M) — its cost was reviewed and **accepted 2026-07-19** for the strong tier; don't swap it out for cost reasons without asking. `gemini-3.7-flash` is cheaper ($0.75/$3.75) and was tried on 2026-08-29, but truncated /api/summarize mid-sentence in production and was reverted the same day. That test turned out to be invalid — summarize.js was the one strong-tier caller not sending `thinkingBudget: 0` at the time — so 3.7 is worth retrying now the cap sits on the chain. Read the comment in `api/_lib/model.js` first.
