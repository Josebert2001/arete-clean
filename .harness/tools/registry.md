...
- **What it does:** Multi-provider fallback for the agentic tutor. `buildModelChain()` returns the providers whose keys are set, in order **Gemini 3.1 Flash-Lite → Groq gpt-oss-120b → OpenRouter `openrouter/free`**. `streamTextWithFallback({ chain, ...streamTextOpts }, onText)` streams text and, if a provider rejects a request *before the first byte*, transparently retries on the next one.
- **Why:** Groq's free tier caps at 8K tokens/request → `413` on big/agentic requests. Gemini 3.1 Flash-Lite's free tier (15 RPM, 1,500 req/day, supports tools) has far more per-request headroom, so those big requests fit; Groq stays a fast fallback; OpenRouter free is a best-effort last resort (no SLA, models can vanish). NOTE: `gemini-3.5-flash` is deliberately NOT used — it's off the free tier and pricey ($1.50/$9.00 per 1M).
