...
- **Why:** Groq's free tier caps at 8K tokens/request → `413` on big/agentic requests. Gemini's free tier (supports tools) has far more per-request headroom, so those big requests fit; Groq stays a fast fallback; OpenRouter free is a best-effort last resort (no SLA, models can vanish). NOTE: the strong tier is a paid model and its cost was reviewed and **accepted 2026-07-19**; don't swap it out for cost reasons without asking. Moved `gemini-3.5-flash` ($1.50/$9.00 per 1M) → `gemini-3.7-flash` ($0.75/$3.75) on 2026-08-29 at the user's request — a cost *reduction*, so the rule above is satisfied rather than bypassed. The 3.7 rate is promotional through 31 Dec 2026.
