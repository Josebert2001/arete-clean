...
Browser → React SPA (Vite)
           ...
           └── /api/* (Vercel serverless)
                ├── api/tutor.js   → model-chain streaming: Gemini strong → Flash-Lite → Groq → OpenRouter (model ids live in api/_lib/model.js — don't restate them here) (AI Tutor, rate: 8/10min/IP)
...
| `src/data/lectureNotes/*Quiz.js` \| `*ExamPrep.js` | The question banks. **7 MCQ (`quiz`)** — UUY-CYB 121, UUY-CYB 123, ENT 221, CYB 224, CYB 222, UUY-CYB 221, UUY-CYB 222 — and **6 written (`examPrep`)** — UUY-CYB 122, COS 221, CYB 224, CYB 222, UUY-CYB 221, UUY-CYB 222. `ent221Quiz.js` is the shared one, imported by both catalogues' ENT 221 entries; the rest sit on their course. These counts feed `courseOfferings()`, so they are quoted verbatim in every public course description and meta tag — a bank added without a rebuild leaves the indexed page understating the course |
...
| `api/_lib/googleAuth.js` | Google OAuth2 client, signed `state` (CSRF), refresh-token storage via `serviceRole.js`. Refresh tokens are **encrypted at rest** (AES-256-GCM) — the column GRANTs govern who may SELECT the value, but a leaked service-role key or a database dump would otherwise hand over every token in plaintext. The key is HKDF-derived from `GOOGLE_CLIENT_SECRET` rather than being a new env var, so it lives in Vercel and never in the database; a dump alone is useless. Legacy plaintext rows are read as-is and re-encrypted lazily on first use, so nothing needs backfilling and nobody reconnects. Rotating `GOOGLE_CLIENT_SECRET` makes existing ciphertexts unreadable — acceptable, since rotating it invalidates the OAuth client anyway |
...
| `vercel.json` | CSP, HSTS, CORS headers, SPA rewrite rule. HSTS is `max-age=63072000; includeSubDomains` — Vercel sends the max-age by default and this restates it with subdomain coverage. `preload` is deliberately omitted: it ships inside browser releases and takes months to reverse, which is not a commitment worth making here |
...
| `src/prerender/entry-server.jsx` \| `vite.ssr.config.js` \| `scripts/prerender.mjs` | **The site's entire search presence.** Areté is a client-rendered SPA behind one `index.html`, and every study route sits behind `RequireAuth` — a crawler asking for `/courses/cyb-224` used to get an empty `<div id="root">` that resolved, after JS, to a redirect to `/signin`, a page `robots.txt` disallows. This step writes **98 files**: `dist/courses/<slug>/index.html` for all 95 courses, the hub, `/install`, and `/` — each with its content already in the bytes, a per-page title/description/canonical/`og:type`, and the page's JSON-LD (`Course` + `BreadcrumbList` + `FAQPage` on a course, `HowTo`×3 on `/install`, `FAQPage` on `/`). It also regenerates `sitemap.xml`, `llms.txt` and `llms-full.txt` into **both** `dist/` and `public/`, so the copies in git can't go stale as courses are added. Runs *after* `vite build` on purpose: VitePWA's precache manifest is generated during that build, so these HTML files are never added to it. Vercel checks the filesystem before applying `vercel.json` rewrites, so the static files win over the SPA catch-all with no routing change. `vite.ssr.config.js` exists separately because running the main config a second time would fire VitePWA's `closeBundle` again and emit a second, wrong service worker.<br><br>**`/` writes `dist/index.html`** — the same file Vercel serves as the SPA fallback for every non-prerendered route. That is deliberate: `main.jsx` clears `#root` on load, so the only request this markup ever answers is one that has not run JS. `collectPages()` takes a `buildDate` from the caller rather than reading a clock, so one deploy's sitemap `lastmod` and its pages' `dateModified` cannot disagree |
| `src/components/HomePreview.jsx` \| `InstallPreview.jsx` | The no-JS faces of `/` and `/install`, the two pages that used to serve an empty `<div id="root">` to every crawler. `Home.jsx` reads AuthContext and calls `useNavigate`; `Install.jsx` holds its three guides behind a `useState` tab picker — neither can go through `renderToStaticMarkup`. Same constraints as `CoursePreview`: no hooks, no context, no react-router. `InstallPreview` renders **all three tracks at once** with `id`s matching the fragment URLs in `installHowToJsonLd()`, because a static file has no tabs |
| `src/data/installGuides.js` | The three toolchain guides as data (`tracks`), plus `installHowToJsonLd()`. Lifted out of `Install.jsx` so the page and `InstallPreview` render the same words from one source. `Install.jsx` still owns the interactivity |
| `src/data/publicNotes.js` \| `src/utils/text.js` | **The freemium slice of the lecture notes** — the only place gated material is public. `loadPublicNotes(course)` returns `outline` (every topic *title* — a table of contents, not the content), `glossary` (≤28 term/definition pairs from the notes' own `definition`/`termlist` sections, the part answer engines actually cite) and `preview` (topic 1's prose, ≤2,400 chars). Everything else stays gated. 14 of 95 courses carry notes; `null` for the rest, so `CoursePreview` renders nothing rather than an empty promise.<br><br>Four filters exist because the extractor cannot tell content from structure, and each was a real defect: **`MIN_DEF_CHARS = 40`** (COS 221 yielded `Simple — "Java is easy to learn."`, COS 121 yielded operator rows — useless attributed to Areté by an engine); **section-number stripping + a structural stop-list** (MTH 121 yielded `2.1 Introduction` as a `DefinedTerm`); **`GLOSSARY_MAX = 28`** (ENT 221's terms are ~378 kB — inlined, the page advertising the notes would be slower than the notes); and **maths exclusion** — inline `$…$` renders through `MathText`, which needs hooks and KaTeX and so cannot run in `renderToStaticMarkup`, and publishing the raw source would put `$f(x)$` into an engine's quote. That costs MTH 121 its whole glossary. **No glossary beats a broken one — do not "fix" this by emitting the raw TeX.**<br><br>`preview.truncated` compares against `topic.sections.length`, *not* the prose sections kept: a topic whose figures and code were stripped has not been shown in full, and the caption a student reads to decide whether an account gives them more must not overstate what they already have. `clip()` moved to `utils/text.js` so this file need not import `publicCatalogue` (that dependency runs one way only — `publicCatalogue` has no knowledge of notes) |
| `src/data/siteMeta.js` | `SITE_URL` / `SITE_NAME` / `INSTITUTION` / `homeTitle()` / `homeDescription()`. A **leaf module with no imports**, re-exported by `publicCatalogue.js` so existing callers don't change. It exists because `usePageTitle.js` — imported by nearly every page — needs the home title now that `/` is prerendered, and importing `publicCatalogue` there dragged the department and note registries into every page's graph (it pushed `coursesPage.test.jsx` past its 5s timeout). Keep this file dependency-free |
...
- **SEO / discoverability:** `/`, `/install`, `/courses` and `/courses/:slug` are the routes open to a logged-out visitor, and they render the `*Preview` components, not the real pages — see the rows on prerendering in the Key Files Map. An inline `<script type="application/ld+json">` is a *data block*, not executable script: the HTML spec's "prepare the script element" algorithm returns before the CSP check for any type that is neither classic nor module, so structured data needs **no** `'unsafe-inline'` and no hash in `vercel.json`. Verified against the real CSP header, not assumed. Do not add a URL to the sitemap without checking it renders something for a signed-out visitor — a listed URL that redirects to the disallowed `/signin` is worse than an unlisted one.
- **AEO / answer engines** (`aeo.md` holds the full audit): three rules the code already follows and that must not be undone.
  1. **An `FAQPage` block's answers must be rendered on the page.** Google treats hidden FAQ markup as a violation and an engine that cannot read the text will not quote it — so `courseFaqs()` / `SITE_FAQS` are one array, rendered by the preview component *and* serialised by `faqJsonLd()`. Never hand-write a second list for the JSON-LD.
  2. **`robots.txt` has no inheritance.** Every agent — `*`, `GPTBot`, `ClaudeBot`, `PerplexityBot`, … — is listed in **one** group. Adding a `User-agent: GPTBot` group of its own silently grants that bot the whole gated surface, because a named group that matches stops the `*` group from applying. `aeo.test.js` fails the build on a second group.
  3. **`llms.txt` / `llms-full.txt` are generated, never hand-edited** (`collectLlmsTxt()` in `entry-server.jsx`). A hand-written manifest claiming "57 courses" is wrong the day a catalogue is added, and a manifest an engine trusts and finds wrong is worse than none. Same two-destination rule as the sitemap.

  4. **Never label a shared course with one department's degree.** `loadPublicCourses()` dedupes shared slugs keeping the Cybersecurity entry, so `{department.degree}` on a public page made all 29 foundation courses — GST 111, MTH 121, STA 111, ENT 221 — announce themselves as "B.Sc. Cybersecurity". Those are the site's highest-volume searches and its widest door, and the label shut it on every other department. `courseAudience()` / `courseAudienceType()` decide this now; `about` is omitted from the Course JSON-LD on a shared course rather than guessed. Areté is a whole-university resource and its public copy has to read that way.

  **Wording rule:** on a `crossDepartmental` course say *"taken across programmes"*, never *"every programme takes it"*. The flag covers alternatives as well as universals (Cybersecurity takes GST 212/312 where Data Science takes GST 211/311), so the stronger claim is checkable and false on some pages. `aeo.test.js` greps for the overclaim and fails.

  A `sameAs` pointing at the wrong entity validates cleanly and teaches every engine a wrong fact — the University of Uyo is **Q7896523** (checked against the Wikidata search API; an earlier draft of `aeo.md` asserted Q1529944, which is a different institution).
