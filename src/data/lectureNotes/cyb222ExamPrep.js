...
export const cyb222ExamPrep = [
...

  {
    ...
    modelAnswer: 'Three-factor authentication requires evidence from three independent categories of proof. They are: something you know — a password, PIN or passphrase; something you have — a registered phone, hardware token or smart card; and something you are — a biometric such as a fingerprint, face or iris. It differs from 2FA in the number of independent categories demanded, not in the number of steps or devices: 2FA draws on two of these categories, typically a password plus a one-time code sent to a registered phone, while 3FA adds the biometric. The categories must be independent for the control to be worth anything — two passwords are not two factors, because an attacker who learns one is very likely to learn the other. Multi-party authentication is a different kind of control. There, the individual originating a request must wait for a second individual holding higher-level credentials to approve it before access is granted; the question is not how confident we are of one person’s identity, but whether one person acting alone should be permitted to act at all. Banking workflows use it for large transfers, which are raised by one officer and authorized by a manager. A familiar consumer example is location-based step-up verification: if a user appears to log in from one country and, within an implausibly short time, a new login attempt appears from another, providers such as Google or Apple may require verification from another trusted device before granting access — because the same credentials used from two impossible locations in a short window is a strong signal of compromise.',
    markScheme: [
      '3FA defined — evidence from three independent categories, not three steps or three devices (2)',
      'Something you know — password, PIN or passphrase (1)',
      'Something you have — registered phone, hardware token or smart card (1)',
      'Something you are — fingerprint, face or iris (1)',
