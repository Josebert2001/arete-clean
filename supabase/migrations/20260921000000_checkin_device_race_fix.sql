-- ─── Device-binding race fix ─────────────────────────────────────────────────
-- Run in the Supabase SQL editor. Idempotent: safe to re-run.
--
-- Fixes a narrow race in check_in()'s device-binding step (introduced in
-- 20260919000000_device_binding.sql, carried into 20260920000000_geofence_
-- toggle.sql): on a student's very first check-in, two concurrent requests
-- from two different devices can both read v_bound IS NULL before either
-- commits. INSERT ... ON CONFLICT (student_id) DO NOTHING then lets the
-- LOSER of that race silently continue with its own p_device_hash — which is
-- NOT the device that actually got bound — instead of failing the way a
-- second check-in from an unbound device should.
--
-- Fix: after the insert, re-SELECT what is actually persisted and compare
-- against that, not against the pre-insert local variable. Everything else is
